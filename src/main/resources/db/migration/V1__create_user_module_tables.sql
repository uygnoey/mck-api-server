...
-- ========================================
-- 5. EVENT_PUBLICATION (Spring Modulith Event Store)
-- ========================================
-- Spring Modulith가 도메인 이벤트를 안정적으로 발행하기 위한 테이블
-- Official schema from Spring Modulith 2.0.1 for PostgreSQL
...
-- Official indexes from Spring Modulith
...
-- Comments
COMMENT ON TABLE EVENT_PUBLICATION IS 'Spring Modulith 이벤트 발행 저장소 (모듈 간 이벤트 통신)';
COMMENT ON COLUMN EVENT_PUBLICATION.LISTENER_ID IS '이벤트 리스너 식별자';
COMMENT ON COLUMN EVENT_PUBLICATION.EVENT_TYPE IS '이벤트 타입 (클래스명)';
COMMENT ON COLUMN EVENT_PUBLICATION.SERIALIZED_EVENT IS '직렬화된 이벤트 데이터';
COMMENT ON COLUMN EVENT_PUBLICATION.COMPLETION_DATE IS '이벤트 처리 완료 시각 (NULL이면 미처리)';
COMMENT ON COLUMN EVENT_PUBLICATION.STATUS IS '이벤트 상태';
COMMENT ON COLUMN EVENT_PUBLICATION.COMPLETION_ATTEMPTS IS '재시도 횟수';
COMMENT ON COLUMN EVENT_PUBLICATION.LAST_RESUBMISSION_DATE IS '마지막 재전송 시도 시각';
