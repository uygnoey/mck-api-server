...
/**
 * Naver OAuth2 클라이언트
 * Naver OAuth2 client implementation
 */
@Slf4j
@Component
...
public class NaverOAuth2Client implements OAuth2Client {
    ...
    public OAuth2TokenResponse exchangeCodeForToken(String code, String redirectUri) {
        ...
        try {
            ...
            if (body == null) {
                throw new BusinessException(ErrorCode.OAUTH_TOKEN_EXCHANGE_FAILED, "Naver token response is empty");
            }
            ...
            log.info("Naver token exchange successful");
            ...
            throw new BusinessException(ErrorCode.OAUTH_TOKEN_EXCHANGE_FAILED, "Naver 토큰 교환 실패: " + e.getMessage());
        }
    }
    ...
    public OAuth2UserInfo getUserInfo(String accessToken) {
        ...
        try {
            ...
            Map<String, Object> body = response.getBody();
            if (body == null) {
                throw new BusinessException(ErrorCode.OAUTH_USER_INFO_FAILED, "Naver user info response is empty");
            }

            // Naver returns user info in a nested 'response' object
            Map<String, Object> responseData = (Map<String, Object>) body.get("response");
            if (responseData == null) {
                throw new BusinessException(ErrorCode.OAUTH_USER_INFO_FAILED, "Naver response data is empty");
            }

            String providerId = (String) responseData.get("id");
            String email = (String) responseData.get("email");
            String name = (String) responseData.get("name");
            String profileImageUrl = (String) responseData.get("profile_image");

            log.info("Naver user info fetched: email={}", email);

            return new OAuth2UserInfo(providerId, email, name, profileImageUrl);

        } catch (Exception e) {
            log.error("Naver user info fetch failed", e);
            throw new BusinessException(ErrorCode.OAUTH_USER_INFO_FAILED, "Naver 사용자 정보 조회 실패: " + e.getMessage());
        }
    }

    private OAuth2Properties.ProviderConfig getConfig() {
        OAuth2Properties.ProviderConfig config = oAuth2Properties.getProvider().get("naver");
        if (config == null) {
            throw new BusinessException(ErrorCode.OAUTH_CONFIG_NOT_FOUND, "Naver OAuth 설정이 없습니다.");
        }
        return config;
    }
}
